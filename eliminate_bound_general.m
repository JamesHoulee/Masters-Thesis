...
function local_kraus (c, primes);
  ...
  for p in primes do
    ...
    for l in [l : l in PrimesInInterval(2*p, Min(p*100, p^4)) | l mod p eq 1 and Denominator(A0*B0*B1*A1) mod l ne 0 ] do
      ...
      gp := PrimitiveElement(k)^p;
      H := [gp^i : i in [0..n-1]] cat [k!0];

      A0H := Set([ k!(A0*u) : u in H ]);
      B0H := Set([ k!(B0*v + 1) : v in H ]);
      ...
      if Numerator(B0) mod l ne 0 then
        B1H := Set([ k!(B1*u + 1) : u in H ]);

        CH := Set([ k!(A1*B0^(-1)*(w-1)) : w in A0H meet B0H ]);
      ...
      if Numerator(A0) mod l ne 0 then
        A2H := Set ([ k!(A2*u) : u in H ]);

        DH := Set([ k!(B2*A0^(-1)*w + 1) : w in A0H meet B0H ]);
